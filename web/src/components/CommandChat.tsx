...
export default function CommandChat({
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { who: 'bot', text: 'Map a piezo buzzer or SG90 servo, then ask me to beep it or move it to a specific angle. I can use only the tools exposed by the connected MCP server.' }
  ]);
  ...
  async function handleSend(e?: React.FormEvent) {
    ...
    try {
      ...
      updateLastMessage(`Sorry, I encountered an error: ${message}. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="card shine flex flex-col gap-3">
      ...
      <form onSubmit={handleSend} className="flex gap-2">
        <input
          className="input"
          placeholder="Try “beep for 500 ms” or “set the servo to 90°”"
          value={text}
